...
drop_ref = {(myref.columns[0], myref.columns[0]),
            (myref.columns[1], myref.columns[1]),
            (myref.columns[1], myref.columns[0])}
corr2ref = myref.corr().unstack().drop(labels=drop_ref)
corr2ref = corr2ref.sort_values(ascending=False, key=lambda col: col.abs())
